...
class ApexTrainer(SimpleTrainer):
    ...
    def __init__(self, cfg, resume=False, reuse_ckpt=False):
        ...
        self.cfg = cfg
        if reuse_ckpt:
            self.reuse_ckpt(model)

        # For training, wrap with DDP. But don't need this for inference.
        if cfg.MODEL.APEX.LEVEL in {'O0', 'O1', 'O2', 'O3'}:
            opt_level = cfg.MODEL.APEX.LEVEL
        else:
            opt_level = 'O0'
        logger.info("Using APEX mixed precision trianing, opt_level={}".format(opt_level))
        ...
        if comm.get_world_size() > 1:
            ...
            if cfg.MODEL.FPN.NORM == 'SyncBN':
                print('converting SyncBN to APEX_SyncBN')
